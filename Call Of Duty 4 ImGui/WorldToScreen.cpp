...
bool w2s::WorldToScreen(Vec3 dst, Vec3 src, ImVec2& ScreenPos, Vec3x3 matrix, Vec3 viewAngles, ImVec2 WindowRes, Vec2 Fov)
{
	Vec3 vLocal, vTransForm;

	AngleVectors(viewAngles, matrix);
	...
	ScreenPos.x = WindowRes.x / 2 + (WindowRes.x / 2 / vTransForm.z * (1 / Fov.x)) * vTransForm.x;
	ScreenPos.y = WindowRes.y / 2 - (WindowRes.y / 2 / vTransForm.z * (1 / Fov.y)) * vTransForm.y;
	return true;
}
